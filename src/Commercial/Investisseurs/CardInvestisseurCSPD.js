...
function CustomCard({ client, user, onCspdCallSelect }) {
  ...
  useEffect(() => {
  console.log(
    "🧠 [CardInvestisseurCSPD] mounted | onCspdCallSelect =",
    onCspdCallSelect
  );
}, []);
    ...
    <CustomCardWrapper style={{ backgroundColor: 'white', borderRadius: '15px', border: 'transparent' }}>
      <CustomCardContent >
        <GlowingBox style={{ backgroundColor: client.CC_BLOQUER ? "red" : "#3572EF", borderRadius: '11px' }}>
          ...
          >
            {safeCapitalize(client.INTITULE_CLIENT)}{" "}
<span style={{ fontSize: "0.8rem", fontWeight: "normal" }}>
  {safeCapitalize(client.INTITULE_GR)}
</span>
        ...
        <Typography variant="h6" component="div" gutterBottom  >
          <PlaceIcon />
          {safeCapitalize(client.ADR_C_FACT_1)}
        </Typography>
        <Typography variant="h6" component="div" gutterBottom  >
          <PlaceIcon />
          {client.SOLDE_CLIENT}
  ...
  const payload = {
    ...
    ID_INVESTISSEUR: client.CODE_CLIENT,
    NOM_PRENOM: client.INTITULE_CLIENT,
...
const CardContainer = ({ searchTerm, onCspdCallSelect }) => {
  ...
  const intituleClient = client.INTITULE_CLIENT || "";
  const intituleGr = client.INTITULE_GR || "";

  return (
    (intituleGr === "Partenaire" ||
      intituleClient.toUpperCase().startsWith("PAR")) &&
    !intituleClient.startsWith("Fam")
  );
});
  ...
  return (
    <div style={{ overflowY: 'auto', maxHeight: 'auto' }}>
      <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between' }}>
        {clients.length > 0 ? (
          clients.map(client => (
            <CustomCard key={client.ID_CLIENT} name={client.INTITULE_GR} client={client} user={user} onCspdCallSelect={onCspdCallSelect} />
          ))
        ) : (
